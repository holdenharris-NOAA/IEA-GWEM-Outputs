...
## Plot catches (NOT WORKING YET)
i=1
grp.num = obsC.head$pool_code[i]
grp = fg_names[grp.num]
pred.idx = which(dimnames(spaC_xY.a2)[[2]]==grp)
#pred.idx = which(unique(spaC_xY.a2.grpnum)==grp.num)
pred = spaC_xY.a2[ , pred.idx, ]
obs = obsC[,i]

#scale obs biomass to bestfit (last) iteration
q = mean(pred,na.rm=T)/mean(obs,na.rm=T)
obs = obs*q

plot(years,obs,pch=16,ylim=c(min(obs,pred,na.rm=T)*0.8,max(obs,pred,na.rm=T)*1.2),cex=0.8,
     xlab='Year',ylab='Catch')
if(is.matrix(pred)){
  matlines(years,pred,lty=1,col=c(rep('gray',ncol(pred)-1),'blue'))
}
if(is.vector(pred)){
  lines(years,pred,lty=1,col='blue')
}
